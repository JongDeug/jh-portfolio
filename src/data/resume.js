  ...
  about: [
    {
      ...
      items: [
        { k: `비용 절감`, v: `여러 SNS 채널을 하나의 병원용 메신저로 묶는 옴니채널 연동을, 외부 SaaS(Respond.io) 대신 SNS 통합 게이트웨이로 자체 구축. 월 최대 약 196만 원(2만 MAC 기준) 운영비 절감(카카오 제외).` },
  ...
  projects: [
      ...
      items: [
        ...
        { k: `쿼리 성능 최적화`, v: `1분 넘게 걸리던 월말 결산·입원 청구 쿼리를 EXISTS 상관 서브쿼리에서 UNION ALL로 재작성해 2초로 단축. 입원 보험집계 쿼리도 인덱스·구조 개선으로 30초 → 5초.` },
        {
          ...
          sub: [
            ...
            `현재 분산 환경엔 비관적 락이 맞지 않다고 판단 → 낙관적 제어(UNIQUE+재시도)·Redis 원자 채번을 대안으로 도출`,
    ...
    },
    {
      ...
      items: [
        ...
        { k: `결정`, v: `컨택팀 약 20명이 매일 쓰는 시스템을 짧은 기간에 신규 스택(NestJS)으로 마이그레이션. 향후 사용 확대를 대비해 다중 서버 수평 확장을 염두에 두고 Redis로 세션·상태를 공유하도록 설계. 내선 전화 중계 서버(NASCA)를 연동해 전화 수·발신 중계 기능을 신규 개발.` },
        { k: `결과`, v: `하루 약 300통의 전화 응대를 버그 없이 처리하고, 미동작하던 웹 발신 기능을 정상화해 컨택팀 만족도 확보. ISMS 취약점 대응(권한 우회 차단 · 개인정보 마스킹 · 세션 재사용 차단)까지 수행.` },
      ],
    },
    {
      title: `병원용 메신저 플랫폼`,
      ...
      items: [
        ...
        { k: `배포`, v: `빌드·태깅하고 실서버에 도커 이미지를 직접 배포하며, 코드가 도는 인프라를 함께 익힘.` },
      ],
    },
    {
